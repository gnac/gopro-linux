...
class GForceWidget(Widget):
    ...
    def render(self, t: float, telem) -> Image.Image:
        ...
        for g_ring in (0.5, 1.0):
            ...
            # Label the ring
            lbl_y = cy - gr - 1
            draw.text((cx, lbl_y), f"{g_ring:.1f}g",
                      font=self._font_g,
        ...
        draw.ellipse(
            [(cx - r, cy - r), (cx + r, cy + r)],
            outline=(*self.grid_color[:3], 255), width=2,
        )

        # Axis labels
        lc = (190, 190, 190, 220)
        draw.text((cx,       cy - r - 2), "ACCEL", font=self._font_lbl, fill=lc, anchor="mb")
        draw.text((cx,       cy + r + 2), "BRAKE", font=self._font_lbl, fill=lc, anchor="mt")
